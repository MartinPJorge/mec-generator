...
#' @description Generate cell antennas within a specified region
#' @param region_json absolute path to the JSON file with the regions locations
#' and census
#' @param regionId identifier of the region within the region_json
#' @param samples number of longitude and latitude samples to grid the region
#' @return list with the intensity matrix, the axis of longitude/latitude
#'         samples list(intenMatrix, lonAxis, latAxis)
generate_cells <- function(region_json, regionId, samples) {
  regions <- fromJSON(file = region_json)
  region <- findRegion(regions = regions, regionId = regionId)
